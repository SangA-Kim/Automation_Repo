...
Parameters: offset - Offset from the beginning of inputDataArray
            len    - Length of String
Description: Functions reads length bytes at specified offset from
             the beginning of inputDataArray and also returns a 
             pointer to the beginning of string following the length
             Bytes
*************************************************************************/
static const char *readLengthAndStringFromInputDataArray(unsigned int currentOffset, unsigned int *len) {
   unsigned char hb = 0;
   unsigned char lb;
   /* Position the pointer at specified offset from the beginning of inputDataArray */
...
/**************************************************************************
Description: This function returns the file descriptors for files 
             opened with write only mode
*************************************************************************/
int vcGetOutputFileID (const char *filename)
{
   int i;
   for (i=0; i < VCAST_MAX_INTERNAL_FILES; i++) {
   
      if(!VCAST_strcmp(filename,vcast_internal_file_names[i]))
 ...
 *************************************************************************/
struct FileData * GetFileFromName(const char * fileName){
  /* Index to the beginning of File Table */
  int index = 0;
...
void vectorcast_fprint_char_hex ( int fpn, char vcast_value ) {
   /* 16-bit char platforms need 7 bytes for "0xFFFF" */
   char local_string[7];
...
void vectorcast_fprint_char_octl ( int fpn, char vcast_value ) {
   /* 16-bit char platforms need 8 bytes for "\177777" */
   char local_string[8];
...
void vectorcast_write_vcast_end (void) {
  ...
  vCAST_END_FILE = vectorcast_fopen  ( "VCAST.END", "w");
...
-------------------------------------------------------------------------------*/
void targetSpecificFileOpen (struct FILES* fd, const char* filename, const char *mode) {
...
int vectorcast_fopen( const char *filename,  const char *mode) {
  ...
  if (VCAST_strcmp(mode, "r") == 0){
    ...
    if ( (VCAST_strcmp(mode, "w") == 0) ||
      ...
      return vcGetOutputFileID(filename);
    }
  }    
...
#else
   if (vectorcast_get_fd (fpn)!=VCAST_NULL)
